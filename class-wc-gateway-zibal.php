...
if (!function_exists('is_plugin_active') || !is_plugin_active('novin-estelam/index.php')) { 
    ?>
    <div class="notice is-dismissible danger-info">
        <p><?php _e( 'جهت استفاده از افزونه زیبال و امکان استعلام شماره کارت, نیاز به فعالسازی افزونه نوین استعلام می‌باشد.', 'text-domain' ); ?></p>
    </div>
    <?php 
    return;
}
        ...
        class WC_Zibal extends WC_Payment_Gateway
        {
            ...
            public function init_form_fields()
            {
                $this->form_fields = apply_filters('WC_Zibal_Config', array(
                        ...
                        'checkCardNumber' => array(
                            'title' => __('تایید شماره کارت', 'woocommerce'),
                            'type' => 'checkbox',
                            'label' => __('جهت اینکه کاربران فقط با شماره کارت‌های تایید شده امکان پرداخت داشته باشند چک باکس را تیک بزنید.', 'woocommerce'),
                            'description' => __('پرداخت فقط با شماره کارت های تایید شده', 'woocommerce'),
                            'default' => 'yes',
                            'desc_tip' => true	
                        ),
                        'zibaldirect' => array(
...
//                $Email = !filter_var($Email, FILTER_VALIDATE_EMAIL) === false ? $Email : '';
                ...
                if ($this->settings['checkCardNumber']) { 
                    $user_ID = get_current_user_id();  
                    $cardnumi = get_post_meta($user_ID, '_send_cardnum', true);
                    $cardnumio = get_post_meta($user_ID, '_send_cardnumo', true);
                    $cardnumit = get_post_meta($user_ID, '_send_cardnumt', true);
                    
                    $allowedCards = array();
                    if ($cardnumi != "") {
                        array_push($allowedCards, $cardnumi);
                    }
                    if ($cardnumio != "") {
                        array_push($allowedCards, $cardnumio);
                    }
                    if ($cardnumit != "") {
                        array_push($allowedCards, $cardnumit);
                    }
                }

                if (isset($allowedCards) && !empty($allowedCards)) {
                    $data = array(
